...
class User < ApplicationRecord
  ...
  INVALID_NAME_CHARACTERS = /\A('|")|('|")\z/.freeze
  ...
  ROLES = %w[admin agent editor user].freeze
  ...
  scope :customers, -> {where('admin = ? and role = ?', false, 'user').where.not(role: %w[agent admin editor]).where.not(id: 2).order('name asc')}
  ...
  def permanently_destroy
    return if is_admin?
    return if id == 2 # prevent the system user from being destroyed

    DeleteUserJob.perform_later(id)
  end

  # Removes or anonymizes associated records
  def scrub
    return if is_admin?
    return if id == 2 # prevent the system user from anonymized
  ...
  def anonymize
    ...
    topics.update_all(user_name: 'Anonymized User')
